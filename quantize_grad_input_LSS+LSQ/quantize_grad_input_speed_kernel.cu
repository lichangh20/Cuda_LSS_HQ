...
// template<typename scalar_t>
// __global__ void dequantize_cuda_kernel(const int32_t * gemm1, const int32_t * gemm2, scalar_t * __restrict__ output_high, scalar_t * __restrict__ output_low, 
...
//     if (x<size){
//        output_high[x] = gemm1[x] * scale_gemm1 + const_x1 * y_col[threadIdx.x];
//        output_low[x] = gemm2[x] * scale_gemm2 + const_x2 * y_col[threadIdx.x];
//     }
// }
...
__global__ void dequantize_cuda_kernel_fp16(const scalar_t * __restrict__ gemm1, const scalar_t * __restrict__ gemm2, scalar_t * __restrict__ output, 
                                        const scalar_t * __restrict__ sum_y_column, const float const_x,
                                        const float scale_gemm1, const float scale_gemm2, int size, int ny){  
    int x = threadIdx.x + blockIdx.x * blockDim.x;
    int row = x / ny, col = x - row * ny;
    scalar_t sumY = sum_y_column[col];
...
std::tuple<torch::Tensor, std::vector<double>, torch::Tensor, torch::Tensor, torch::Tensor, double> quantize_cuda(torch::Tensor x, torch::Tensor qy, float scaley, torch::Tensor y, int num_bins, torch::Tensor hadamard_activation, torch::Tensor scale_activation, torch::Tensor activation_phi_in){
    ...
    auto vec_norm = torch::cat({torch::mul(x1_len, I_len), torch::mul(x2_len, I_len)});
    ...
    // sample process
    // auto qx_sample = torch::cat({first_transform, second_transform}, 0);
    // int half_shape = qx_sample.size(0) / 2;
    // auto split_xlist = torch::split(qx_sample, {half_shape, half_shape}, 0);
    // auto sample_x1 = split_xlist[0].contiguous();
    // auto sample_x2 = split_xlist[1].contiguous();

    auto sample_x1 = first_transform / norm_activation_loop.index({Slice({None, len_norm/2})}).unsqueeze(1);
    auto sample_x2 = second_transform / norm_activation_loop.index({Slice(len_norm/2)}).unsqueeze(1);
    ...
    // pack_cuda_kernel<<<grid_pack_y,block>>>(qy.t().contiguous().data_ptr<int8_t>(), sample_y_int4.data_ptr<int8_t>(), grid_size_y);
    ...
    float const_x1 = (8.0 / scale1 + zero_point1);
    float const_x2 = (8.0 / scale2 + zero_point2);
    float const_x = const_x1 + const_x2;
    float scale_gemm1 = 1. / (scale1 );
    float scale_gemm2 = 1. / (scale2);
    int size = nx*ny;
    ...
    dequantize_cuda_kernel_fp16<scalar_t><<<grid2, block>>>(
        ...
        sum_y_column.data_ptr<scalar_t>(),
    ...
    // //TODO: sample2, select half result from output_high and output_low
    // auto Ind_high = torch::zeros_like(output_high);
    // auto Ind_low = torch::zeros_like(output_low);
    // Ind_high.index_put_({Slice(0, int(Ind_high.size(0) / 2))}, 1);
    // Ind_low.index_put_({Slice(int(Ind_high.size(0) / 2))}, 1);
    // grad_output = output_high.mul(Ind_high) + output_low.mul(Ind_low);
    ...
    double sample_time = (double)((time_sample_end - time_leverage_end) + (time_sample2_end - time_dequantize_end)) / CLOCKS_PER_SEC;
    ...
    return std::make_tuple(gemm1, time_vector, norm_activation, grad_output, grad_alpha, grad_scale);
}
